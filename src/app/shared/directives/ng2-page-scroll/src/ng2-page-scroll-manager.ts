...
export class PageScrollManager {
    ...
    private static runningInstances: PageScrollDirective[] = [];
    ...
    public static add(pageScroll: PageScrollDirective) {
        this.runningInstances.push(pageScroll);
    }

    public static stopAll(): boolean {
        if (PageScrollManager.runningInstances.length > 0) {
            PageScrollManager.runningInstances.forEach((pageScroll: PageScrollDirective, index: number) => {
                pageScroll.stop();
            });
            return true;
        }
    ...
    public static remove(pageScroll: PageScrollDirective): boolean {
